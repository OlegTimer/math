...
cout<< "Calculate center of gravity/symmetry with double integral_D f(x;y)dxdy\n";
...
cout<< "For example, D: y=x^2 +4x+3; y=0; center of gravity is: (-2\t-0.4)\n\n";
...
result.push_back("Center of gravity with double integral_D f(x;y)dxdy with D domain:\n");
for (int i=0; i<f.size(); i++){
t=(int) f[i][0]; 
d=f[i][1];
a=f[i][2];
...
long double in, sq,sqx,sqy; //calc defin integral by middle squares
string sqstr("");
string sqxstr("");
string sqystr("");
in=0;
...
sqstr =nt;	  
result.push_back(" = "+nt+"\n");
result.push_back("Sq = Square of plain figure with D Domain = "+nt+"\n");

result.push_back("\nIf f(x;y)=x \n");
s="inner integral is: "+inner;
result.push_back(s+"\n");
result.push_back("Proceed to definite integral: \n");
s="integral from "+xmins+" to "+xmaxs+" x*( "+inner+" ) dx";
result.push_back(s+"");
in=0; 
for(double i = xmin+D/2; i<xmax; i+=D ){
in+=(d*(i*i*i*i) + a*(i*i*i) + b*(i*i) + c*i)*D;
	  }
sqx=in;	  
strs.str("");
strs << in;
...
sqxstr=nt;	  
result.push_back(" = "+nt+"\n");
result.push_back("Sqx = "+nt+"\n");

result.push_back("\nIf f(x;y)=y \n");
result.push_back("Proceed to definite integral: \n");
s="0.5 * integral from "+xmins+" to "+xmaxs+" ( ("+sf1+")^2 - ("+sf2+")^2 ) dx";
result.push_back(s+"");
in=0;
long double topres, btmres;
for(double i = xmin+D/2; i<xmax; i+=D ){
topres = d1*i*i*i + a1*i*i + b1*i + c1; 
btmres = d2*i*i*i + a2*i*i + b2*i + c2; 		   
in+=(topres*topres - btmres*btmres)*D;
	  }
in=in/2;	  
sqy=in;	  
strs.str("");
...
result.push_back("\nFor center of gravity: x=Sqx/Sq; y=Sqy/Sq\n");
long double xcent, ycent;
xcent = sqx/sq;
ycent = sqy/sq;
in = xcent;
strs.str("");
strs << in;
nt = strs.str();
result.push_back("Center of gravity is: ("+nt+"\t");
in = ycent;
strs.str("");
strs << in;
nt = strs.str();
...
//we can add center of gravity poind/dot to graph
int xn=0;
in = ((xcent-xmin)/(xmax-xmin))*console_line_length;
xn = (int) in;
if (xn<0){xn=0;}
if (xn>console_line_length-1){xn=console_line_length-1;}
in = (1-(ycent-ymin)/(ymax-ymin))*console_line_num;
yn = (int) in;
if (yn<0){yn=0;}
if (yn>console_line_num-1){yn=console_line_num-1;}
matrix[yn][xn]='X';
