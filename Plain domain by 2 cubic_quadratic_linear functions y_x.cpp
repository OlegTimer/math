...
cout<< "Get domain (plain figure), created with 2 functions y(x) \n";
...
cout<< "\nNote: first value is function type: (0 - no function; 1 - y; 2 - x)\n";
...
c=f[i][4];
if (t==1)				{	
result.push_back(funcshow(t,d,a,b,c)); 
result.push_back("\n");	
						}
}
...
if (m1<m2){top=2;}
int bottom;
if (top ==0){bottom=2;}else{bottom=0;}

string tmp = result[bottom];
string sub = tmp.substr(4, 80);
result.push_back(sub);
result.push_back(" <y< ");
 tmp = result[top];
