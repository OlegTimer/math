...
cout<< "Calculate double integral_D f(x;y)dxdy  \n";
cout<< "for 2 integrand (integral's function) f(x;y)=1 and  f(x;y)=x\n";
...
cout<<"With f(x;y)=1 integral is a square of D domain (plain figure)\n\n";

cout<< "For example, f(x;y)=1; with D y=x^2 -1; y=5-x; double integral_D dxdy =20.83\n";
cout<< "For example, f(x;y)=x; with D y=x^2 -1; y=5-x; double integral_D xdxdy=-10.4\n\n";
...
result.push_back("Double integral_D f(x;y)dxdy with D domain:\n");
for (int i=0; i<f.size(); i++){
t=(int) f[i][0]; 
d=f[i][1];
a=f[i][2];
